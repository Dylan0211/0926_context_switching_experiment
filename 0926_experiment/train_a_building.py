...
def get_mae_for_24h_cl(generated_data, target_data, is_weekend):
    """
    xxxx
    cal_generated_mae_for_best_compare
    :param generated_data:  xxxxxx shape=[80, ]
    :param target_data: shape=[80, ]
    :param is_weekend: is_weekend_flag
    :return:
    """
...
def check_training_result(gen_a, gen_b):
    with open('../tmp_pkl_data/{}_ashrae_{}_to_{}_data_dict.pkl'.format(a_building_name, context_a, context_b), 'rb') as r:
    ...
    print('test on: {}'.format(a_building_name))
...
def train_GAN_on_a_building():
    with open('../tmp_pkl_data/{}_ashrae_{}_to_{}_data_dict.pkl'.format(a_building_name, context_a, context_b), 'rb') as r:
        save_dict = pickle.load(r)

    # get context a and context b data
    ...
    torch.save(best_gen_a.state_dict(), a_gen_a_save_path)
    torch.save(best_gen_b.state_dict(), a_gen_b_save_path)


if __name__ == '__main__':
    train_GAN_on_a_building()
