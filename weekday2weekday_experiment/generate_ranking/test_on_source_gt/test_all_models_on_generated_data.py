...
def test_model():
    ...
    for index, model_name in enumerate(model_name_list):
        # load data
        with open('./tmp_pkl_data/{}_ashrae_{}_gt.pkl'.format(target_building_name, source_context), 'rb') as r:
    ...
    # output as csv file
    with open('./test_result/test_on_{}_ashrae_{}_gt_result.csv'.format(target_building_name, source_context), 'w', newline='') as f:
        write = csv.writer(f, dialect=('excel'))
        write.writerow(['rank', 'model_name', 'MAE'])
        for index, model in enumerate(sorted_models):
            write.writerow([index + 1, model[0], model[1]])
    print('result saved at: ./test_result/test_on_{}_ashrae_{}_gt_result.csv'.format(target_building_name, source_context))
