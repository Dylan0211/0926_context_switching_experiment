...
from weekday2weekday_experiment.generate_ranking.model import *
...
def test_model():
    ...
    for index, model_name in enumerate(model_name_list):
        # load data
        with open('./tmp_pkl_data/{}_ashrae_{}_to_{}_generated_data.pkl'.format(target_building_name, source_context, target_context), 'rb') as r:
        ...
        # fig = plt.figure(figsize=(10, 6))
        # fig.add_subplot(111)
        # plt.plot(range(len(data_y)), data_y, color='blue')
        # plt.show()
        ...
        # fig = plt.figure(figsize=(10, 6))
        # fig.add_subplot(111)
        # plt.plot(range(len(label_list)), label_list, color='blue')
        # plt.show()
    ...
    with open('./test_result/test_on_{}_with_{}_GAN_ashrae_{}_to_{}_result.csv'.format(
            target_building_name, GAN_building_name, source_context, target_context), 'w', newline='') as f:
    ...
    print('result saved at: ./test_result/test_on_{}_with_{}_GAN_ashrae_{}_to_{}_result.csv'.format(
        target_building_name, GAN_building_name, source_context, target_context))
