...
def check_training_result(gen_a, gen_b, epoch):
    ...
    """
    mae_list = []
    for index, building_name in enumerate(b_building_names):
        ...
        if index == 0:
            ...
            plt.savefig("{}/train_set_b_epoch_{}.png".format(train_result_save_path, epoch))
...
def train_GAN_on_b_buildings():
    ...
    cl_a_list = []
    cl_b_list = []
    for building_name in b_building_names:
        with open('../tmp_pkl_data/{}_ashrae_{}_to_{}_data_dict.pkl'.format(building_name, context_a, context_b),
                  'rb') as r:
            save_dict = pickle.load(r)
    ...
    torch.save(best_gen_a.state_dict(), b_gen_a_save_path)
    torch.save(best_gen_b.state_dict(), b_gen_b_save_path)


if __name__ == '__main__':
    train_GAN_on_b_buildings()
